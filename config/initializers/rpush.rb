Rpush.configure do |config|
  ...
  # TODO: make the paths into shared directory
...
Rpush.reflect do |on|
  ...
  on.gcm_canonical_id do |old_id, canonical_id|
    device = V1::Device.find_by(gcm_registration_id: old_id)
    if device.nil?
      return
    end

    new_device = V1::Device.find_by(gcm_registration_id: canonical_id)
    if new_device.nil?
      device.update_attributes(gcm_registration_id: canonical_id)
    else
      device.destroy
    end
  end
  ...
  on.gcm_invalid_registration_id do |app, error, registration_id|
    ...
    V1::Device.where(gcm_registration_id: registration_id).destroy_all
  end
